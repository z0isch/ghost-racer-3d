...
Three of the four counts are spent inside a race and are meaningless outside one; the income ghost count is the exception, and is the only part of a loadout that does anything while the circuit is not being driven.
...
**Income ghost**:
A translucent green replay of a circuit's ghost line that runs it forever, earning what that line would earn if you drove it today. Alone among the ghosts it is not something you meet while racing: it exists precisely when you are *not* driving the circuit, and is hidden the moment you enter one. Its count comes from the **circuit loadout** exactly as the boost and hazard counts do, and starts at 0 for the same reason.
...
It wraps by popping back from the start/finish gate to the start line each lap, the same few metres the lap cycle itself covers behind a countdown. Not blended — the ghost line's whole claim is that it is a recording, and blending would invent motion nobody drove.
A circuit with no ghost line has nowhere to run one, so a bought income ghost earns nothing and is not drawn, exactly as there are no boost ghosts on lap 1.
Green, and green for a reason: the purse and the pickup popups are already green, and an income ghost is the only car in the game that *is* the money. It shares the pace ghost's silhouette and direction of travel, which would be a collision if the two were ever on screen together — and they never are, since one exists only in a race and the other only outside one.
_Avoid_: Lap ghost ("lap" is spoken for four times over already — see **Lap earnings** especially), idler, worker, farmer, AFK ghost.

**Income**:
What an income ghost earns, in dollars. Not replayed from what the record lap earned, but **re-derived from that line against the coins live today**: the recorded path is swept against the circuit's currently bought coin field, and each coin it crosses pays that coin's own value at the moment it is crossed. Buying a coin the line runs through therefore raises income the instant it is bought, with no need to drive the circuit again — and the money shown stands where the coins actually are rather than being mimed on a timer, which is the same honesty the pace ghost is held to.
Each ghost carries its own idea of which coins it has taken, cleared when its own lap wraps. Ghosts do not compete for coins, and cannot take one from each other or from you: N ghosts pay N times, and income is exactly linear in the count. Balancing that belongs in what a ghost costs, never in a rule that quietly makes the third ghost worth less than the first.
**Income is not an earn rate.** Both are dollars per second, and the two must never be compared or added: the earn rate measures how well you drove and is the only thing that can set a record, where income measures what you have bought. Nothing an income ghost does can promote a ghost line, set a record, or touch the coin field of a lap you are driving.
_Avoid_: Passive income (fine in conversation, too long as a term), wage, yield, revenue, earnings (spoken for by **Lap earnings**), idle money.

Income arrives silently. It never flashes the purse readout, because a flash that fires whether or not the player did anything stops meaning "you took a coin" — see **Purse readout**. In the open world it is visible instead as the ghosts themselves and the popups they leave behind; inside a race it has no visible sign at all beyond a total that climbs faster than the coins alone explain.
