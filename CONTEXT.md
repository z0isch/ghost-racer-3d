...
**Circuit loadout**:
What one player has bought — and, since **Tune**, earned — for one circuit: how many clocks are live on it, how many boost ghosts, how many hazard ghosts, how many income ghosts, and how much top speed it has earned. Raw counts rather than levels — a level is a second representation of a count, and a second representation is a thing that can disagree with the first — and every one of the bought counts starts at 0, so a circuit is bare until it is invested in. A bare circuit still pays, because its checkpoints do; what a bare circuit has none of is decisions. The Tune is not a count, and does not start bare the way the counts do the moment a circuit has been raced at all — see **Tune**.
Deliberately not part of the **Circuit**. A circuit's definition is authored content, identical on every machine and versioned with the game; a loadout is one player's purchases (and earnings), and belongs beside their ghost lines rather than in the repo. It persists exactly as a ghost line does — one per circuit, found by a path the circuit itself carries — so a new circuit is still one authored resource, with no central registry to also remember to edit.
...
Three of the four bought counts are spent inside a Run and are meaningless outside one; the income ghost count is the exception, and is the only part of a loadout that does anything while the circuit is not being driven. It is also the only one that raises **income** directly — the clock count reaches income the long way round, by making a longer Run possible and a higher record with it.
...
**Tune**:
The permanent gain in the kart's top speed a circuit has earned, in m/s. Awarded when a Run takes the record off a standing **pace ghost** (see **Ahead of pace**) — and only then: a circuit's first completed Run sets a record with no ghost on the track, and beats nothing, so it earns nothing. A **Wreck** and a **Rewind** are no bar to it, exactly as they are no bar to the record itself.
Per circuit, and kept in the **circuit loadout** beside the four bought counts, though it is the one thing there that was driven for rather than paid for. It has no effect in the **open world**, which has no circuit to have earned one.
It is added onto the same ceiling a **slipstream ghost** catch raises and is clamped by the same `max_top_speed`, which means the two share one pool: on a circuit with slipstream ghosts bought, a Tune reads as reaching top speed sooner rather than as a faster car, and it is at its most visible on a circuit nothing has been bought for yet. Deliberate — one ceiling, one number, and the Tune stops being awarded once it fills the room it has.
Said out loud exactly once, in a gold line under the record headline on **Results**, on the Runs that earn one. Never a fifth ranked figure: it is a consequence of the record, not a measurement of the Run.
_Avoid_: Tuning (`KartTuning` and `tuning/default.tres` are authored dials, identical on every machine; a Tune is one player's earnings on one circuit), upgrade (nothing is bought), boost, top speed bonus (that is the per-Run gain a slipstream catch banks, which a Tune stands beside).
...
Money, not a score. The pace ghost is promoted on track position and never on this, and nothing is won by hoarding it — what it is *for* is the circuit loadouts it buys. It was a monotone lifetime total for exactly as long as nothing consumed it; the shop is that consumer.
...
**Earn rate**:
Run earnings divided by the Run clock, in dollars per second. What Results and RunHud show as the Run's figure of merit, and what an income ghost pays — but not what decides the record any more (see **Ahead of pace**): that is track position. It is a cumulative average over the Run, not a windowed or instantaneous figure.
...
**Record earn rate**:
The current pace ghost's own earn rate — not a bar anything is measured against, since promotion is no longer decided on it (see **Ahead of pace**), but still the figure RunHud shows as "RECORD" and the one the Results screen's delta column compares against. The record and the ghost are one concept with two faces — by construction it is always the earn rate of the Run that *is* the pace ghost, so there is never a moment where the number on screen and the car beside you disagree.
It is also, exactly, what each of the circuit's income ghosts pays per second — see **Income** — which is now a consequence of whichever Run happened to take the track-position record, not of that Run necessarily having driven the best rate anyone has managed this session.
_Avoid_: Best lap, high score, personal best.

**Ahead of pace**:
Whether the kart's own position on the circuit — however many wraps it has closed, plus how far round the current one its nose sits, both measured by dropping a perpendicular onto the circuit's centreline — is further round the track than the pace ghost's position at that same instant of the Run clock. The test a Run's finishing position is put to at Timeout or Wreck to decide whether it takes the record (`RunDirector.complete_run`), and the same test read continuously while driving, for a HUD that wants to say "ahead" or "behind" without waiting for the result.
Replaces earn rate as the promotion test: two Runs of different lengths (a longer one having taken more clocks) are earn-rate-comparable but not track-position-comparable in the same intuitive way a live race is, which is deliberate — this is a test of "who got further, right now" rather than "who earned faster over their own stretch of time". Earn rate keeps its old jobs — it is still what Results and RunHud show, and still what an income ghost pays — but it no longer gates the ghost line.
_Avoid_: In the lead, winning (there are no other racers, only the one ghost).
...
Because promotion demands finishing strictly further round the track (see **Ahead of pace**), the ghost goes **stale on purpose**: set a strong Run and it may stand for fifty more. That is the intended feel. The ghost is a standard to be beaten, not a mirror of what you just did, and it stops moving the moment it becomes hard to beat.
...
**Run director**:
The single owner of all mutable Run state — the run phase, checkpoint progress, the **checkpoint ladder**'s current rung, the Run clock, **Condition**, the slipstream ghosts caught this Run, Run earnings, the record earn rate, whether the Run is currently **ahead of pace**, and the ghost line itself, both the in-progress recording and the promoted line. Everything else in the game either reports to it or reads from it. (The pace ghost reads the ghost line for pure playback and the clocks live on the clock field, which the director drives through run events; authority over a lifecycle is not a requirement that every consumer's data be fields on the director.)
...
**Income is not an earn rate.** They are now numerically equal for the record Run, which makes the distinction matter more rather than less: the earn rate measures how well that Run happened to earn, where income is what its rate pays while you are elsewhere, multiplied by a count you bought. The record itself is set on track position, not earn rate (see **Ahead of pace**) — a Run can take the record on a rate no better than one that lost, and income is paid at that rate regardless. Nothing an income ghost does can promote a ghost line or set a record.
