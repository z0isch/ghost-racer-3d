...
**Not a unit of play.** Nothing is scored per wrap, no wrap time is shown, there is no wrap record, and the **checkpoint ladder** runs straight through one without resetting. It is a unit of *recording* and of *restoration*: the **ghost line** stores every checkpoint crossing, and the first *checkpoint count* of them mark the first wrap — which is how the hazard field finds one wrap's worth of line to place itself along. Where a Run's budget ran out before a single wrap closed there are not enough of them to mark one, and the hazard field takes the whole partial recording instead: traffic on the piece of line that exists beats no traffic at all, and a circuit long enough to outlast its own Run would otherwise never show a hazard. The **boost ghost** field restores and re-rolls on the same signal, but from the road's own centreline rather than this line. Where the countdown used to restore a field, the wrap now does, for both — though only the boost field is *re-rolled* by it, since a **hazard ghost** drives and so re-places itself continuously.
...
A **clock** is taken too, and so is a **boost ghost**, by the same swept path. The shared verb is deliberate and is a true statement about the design, not a coincidence: all three are things the kart's path collects rather than touches, and all three are tested against the segment travelled rather than a sampled position. A checkpoint comes back on the next wrap; a clock and a boost ghost do not come back until the next Run — the **hazard ghost** is the one field restored per wrap rather than per Run.
...
**Clock field**:
Every *live* clock on the circuit, considered as one thing. A purely spatial concern — where the clocks are and which are currently taken — and the whole of it is restored at the start of every countdown, so each Run is offered an identical maximum.
It is *not* restored on a **wrap**, and that is load-bearing rather than a nicety: the clock field is per-Run — as the **boost ghost** field now is too — where the **hazard ghost** field is per-wrap, and since the earn rate grows without bound in Run length, an indefinitely extendable Run would have an unbounded record. A circuit's authored clock seconds are the ceiling on how long its best Run can be and, through that, the ceiling on its record.
...
**Hazard ghost**:
A translucent, red-tinted ghost of the car driving the ghost line **backward** — oncoming traffic on your own best line — **hit** on contact, the same swept test a clock or a boost ghost is taken by, and hit once is hit for the rest of the **wrap**. Placed automatically along one wrap's worth of the ghost line — or along the whole recording, where no wrap ever closed inside it — one per equal slot of that stretch, exactly as boost ghosts are — the difference is that a hazard ghost stands *on* the line rather than a step to one side, since there is nowhere else for oncoming traffic to be, and it drives rather than stands still. Every hazard on a circuit costs the same, so the count is a known quantity. It comes from the **circuit loadout** exactly as the boost ghost count does, and starts at 0 for the same reason.
**A wrap restores the field but does not re-roll it**, and that is where it parts company with the **boost ghost**. A boost ghost is consumed and then stands still, so nothing short of a re-place brings it back or moves it; a hazard drives the whole wrap continuously under its own speed, so it re-rolls itself and has nothing to gain from being freed and respawned. What that would cost is real: oncoming cars popping out of existence and a fresh field appearing in front of a driver already at speed — which a countdown can do unseen from a standing start and a wrap crossed at pace cannot — and the field's start margin re-cut at the line every wrap, leaving a hole in the traffic there for the length of the Run.
