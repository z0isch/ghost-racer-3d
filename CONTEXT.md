...
**Not a unit of play.** Nothing is scored per wrap, no wrap time is shown, there is no wrap record, and the **checkpoint ladder** runs straight through one without resetting. It is a unit of *recording* and of *restoration*: the **ghost line** stores every checkpoint crossing, and the first *checkpoint count* of them mark the first wrap — which is how the hazard field finds one wrap's worth of line to place itself along. The **boost ghost** field restores and re-rolls on the same signal, but from the road's own centreline rather than this line. Where the countdown used to restore a field, the wrap now does, for both.
...
A **clock** is taken too, and so is a **boost ghost**, by the same swept path. The shared verb is deliberate and is a true statement about the design, not a coincidence: all three are things the kart's path collects rather than touches, and all three are tested against the segment travelled rather than a sampled position. What differs is only when a taken thing comes back — a boost ghost on the next wrap, a clock not until the next Run.
...
It is *not* restored on a **wrap**, and that is load-bearing rather than a nicety: the clock field is per-Run where the boost and hazard fields are per-wrap, and since the earn rate grows without bound in Run length, an indefinitely extendable Run would have an unbounded record. A circuit's authored clock seconds are the ceiling on how long its best Run can be and, through that, the ceiling on its record.
...
It is no longer one lap long, and every consumer has to be read in that light. A Run's recording runs for the whole time budget and ends mid-corner wherever the Timeout fell, so it is neither loop-closed nor of a predictable length: the checkpoint crossings are what let the hazard field find one wrap's worth of it to work with, and the unclosed end is what an income ghost pops across. A saved line from before the crossings existed has none, and is treated as a circuit that has never been driven — it clears itself the first time someone sets a record.
...
**The wrap is what the countdown used to be.** The field is restored — and **re-rolled** — at every wrap rather than only at every countdown, so a long Run is not one live wrap followed by an empty circuit, and no two wraps of a Run are the same track. Each ghost is drawn inside its own slot of the centreline and jittered inside it, which is what keeps a ghost a decision from Run to Run rather than a fixture, and what keeps the field from settling into the same shape for the fifty Runs a stale centreline can stand — the centreline itself never moves, since it is the road's, not the driver's.
_Avoid_: Boost pad, boost panel, ramp, speed strip, respawning (a boost ghost comes back on a wrap, but never within one).
