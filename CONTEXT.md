...
It is *not* restored on a **wrap**, and that is load-bearing rather than a nicety: the clock field is per-Run — as the **boost ghost** and **hazard ghost** fields are too — and since the earn rate grows without bound in Run length, an indefinitely extendable Run would have an unbounded record. A circuit's authored clock seconds are the ceiling on how long its best Run can be and, through that, the ceiling on its record.
