...
## bought clocks out there. One implementation, so a change to how a material is dimmed — a
...
## which is how a gate's material and a clock's are respectively found) and returns a copy at [param
...
## (a gate's own override, or the [TorusMesh] every clock in a circuit points at) is never
## touched.
